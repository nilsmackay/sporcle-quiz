...
export default function SetupRounds({ enabledRounds, setEnabledRounds, believeItStatementCount, youtubeVideoCount, pictureRoundCount, sampleHitsterSongCount }) {
  return (
    <div className="game-card p-5 mb-6">
      ...
      <div className="space-y-2">
        <label
          htmlFor="round-believe-it"
          className={`choice-card flex items-center gap-4 p-4 cursor-pointer ${enabledRounds.believeIt ? 'selected' : ''}`}
        >
          <div className={`w-6 h-6 border flex items-center justify-center transition-all ${
            enabledRounds.believeIt
          ...
          <input
            type="checkbox"
            id="round-believe-it"
            name="round-believe-it"
            checked={enabledRounds.believeIt}
            onChange={(e) => setEnabledRounds({ ...enabledRounds, believeIt: e.target.checked })}
            className="sr-only"
          />
          <div className="flex items-center gap-3 flex-1">
            <span className="editorial-stamp w-7 h-7 text-xs flex-shrink-0">1</span>
            <div>
              <span className="font-bold text-[#1A1A1A] block">Can You Believe It?</span>
              <span className="text-sm text-[#6B6560]">True or false? ({believeItStatementCount} statements)</span>
            </div>
          </div>
        </label>
        ...
        >
          ...
          />
          <div className="flex items-center gap-3 flex-1">
            <span className="editorial-stamp w-7 h-7 text-xs flex-shrink-0">5</span>
            <div>
